...
#生成方块
execute as @e[tag=yumo_cmp_item_frame_cl] at @s run function yumo_cmp:cl/item/spawn

#标记实体
execute as @e[tag=yumo_cmp_cl_item_marker] unless data entity @s {data:{yumo_cmp_Item_load:{stop:1b}}} at @s run function yumo_cmp:cl/item/marker/marker
